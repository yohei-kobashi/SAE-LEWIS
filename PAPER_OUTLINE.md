...
> ⚠️ **下表は共有rngバグ(§6e-1)の影響下の値** — ef32/steer/steer_rnd は
> gold順を再利用しており FRR が系統的に膨張、routedのみ無バイアス。膨張は
> 「ef32がFRR首位」の向きに効くため、**この表の順位は `run_frr_rerun.sh`
> による再判定まで暫定**。exact列はjudge非依存なので影響なし。
...
system判定のみ取り直す(`run_frr_rerun.sh`、旧ファイルは `*.orderbug.jsonl` に退避し
`judge_agreement.py` で新旧を突き合わせ → **FRR(旧)−FRR(新) = 位置バイアス
膨張量の実測**)。

### 6e-2. 自己一致率(バグ影響下の暫定値。routed列のみが正直な読み)

| judge | routed列(順序ランダム化=正) | ef32/steer列(gold順再利用=無効) | 非exact FRR |
|---|---|---|---|
| **GPT-4o** | **0.9926 [0.973, 0.998]**(flips 2/271) | 0.9953 / 0.9956 | 0.7687 |
| gemma-2-9b-it | 0.9517 [0.919, 0.972](flips 13/269) | 1.0000 / 1.0000(構成上) | 0.6849 |
| gpt-5.4-nano | 0.8893 [0.846, 0.921](flips 30/271) | 0.9120 / 0.9058 | 0.6761 |

- **GPT-4oが最も自己一致的で、CIが他2judgeと分離** → 主judge選定の実証的
  根拠。LinguaLens整合という外形的理由と、測定器としての品質が一致した。
- nanoは **ef32/steer列(gold順再利用)でも19–21 flips** — 決定的なはずの
  条件で矛盾する = 純粋なAPI非決定性。gpt-5.x系は `temperature≠1` を拒否
  するため `temperature=1` サンプリングでしか判定できず、これは回避不能な
  制約。**nanoを主judgeにできない技術的理由**として書ける。
- **減衰シグネチャ(強い傍証)**: judge信頼性がFRRの水準とシステム間分離を
  単調に予測する — GPT-4o(SC 0.9926)平均FRR 0.8177・ef32−steer **13.1pt**、
  gemma(0.9517)0.7786・5.6pt、nano(0.8893)0.7437・5.8pt。ノイズは差を
  チャンス方向へ**減衰**させるだけで捏造しない。最も静かな測定器が最大の
  差を見せる=**差は実在し、GPT-4oが最良の測定器**。
- judge間一致率(realized判定、n≈963–973): routed gemma-GPT4o **0.8654** /
  GPT4o-nano 0.8193 / gemma-nano 0.7812。ef32 0.8592/0.8235/0.8111、
  steer 0.8839/0.8017/0.7957。gold方向一致 0.8867/0.8074/0.8044 —
  **自己一致率上位2つ(GPT-4o・gemma)が互いに最も一致**し、nanoが外れる。
- 注意: 自己一致 ≠ 正確さ(一貫して間違うjudgeはあり得る)。主張は
  「GPT-4oが最良の測定器である傍証」までに留め、正確さは主張しない。

**未決(再判定待ち)**: GPT-4oの位置感度は0.74%と小さいので ef32−routed
4.6pt は生き残る見込みだが、gemmaは4.8%あり 5.9pt のgapが消える可能性が
ある。§6cの「FRR首位はjudge不変」は**再判定まで暫定**。
