...
| | **本研究 (SAE-EditFlow)** | **LinguaLens** | **AxBench** |
|---|---|---|---|
...
ことをまさに行っている。SAE-EditFlowの差分は「**仕様がモデル内部から回収された
...
2. **粒度**: Tan et al.の anti-steerability は**データセット内のper-input
   比率**(データセット平均のsteerabilityは概ね正のまま)。SAE-EditFlowの
...
**SAE 0.695** / PCA 0.652。
**SAE-EditFlowが教師なしで特徴選択するなら該当するのは0.695の方**。
...
(= SAE-EditFlowが使う `gemma-scope-2b-pt-res` 系そのもの)を使用**。
...
あり、SAE-EditFlowの表層離散編集とはタスクが異なる」という切り分けは **0-3で
...
→ **対比は「LinguaLensは現象あたり3本のbase vectorで介入」 vs 「SAE-EditFlowは
...
条件付ける」が、その**仕様は表層の用語辞書/トークン列**である。SAE-EditFlowの
