...
- **介入手法の詳細(2026-07-23、arXiv HTML抽出 — camera-ready照合は
  ...
  **IE勾配帰属は我々のE'(editor読み出し)の方法論的先行 — E'実施時に
  必ず引く**。
- **🚫 書いてはいけない(2026-07-23ユーザー質問で確定)**: 「Marksは
  介入ではない」— activation patchingのdo(a=a_patch)は教科書的な因果
  介入であり、検証段のmean/zero ablationは実介入。正しくは「**発見段は
  介入の勾配近似・実介入は検証段のablationに限られ、成功基準は一貫して
  挙動(logit/分類器)でテキスト編集の実行はない**」。同様にBrinkmannを
  「ablationのみ」と書かない — 翻訳実験は反実仮想値クランプ(足す方向)
  もある。正確な括り=「**LinguaLens同様の固定値クランプの域を出ない**
  (ゼロ化/定数設定・手動強度・最終トークン一律)、指定featureを
  テキスト上の該当構造にピンポイントに実現する編集はない」。
