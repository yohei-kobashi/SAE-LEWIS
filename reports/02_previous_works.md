...
> **⚫ 改訂ガイド(2026-07-17 EF完全除外後 — 最優先)**: 論文のRelated
> Workの背骨は **軸5(因果評価とSAE評価ベンチマーク、本ファイル末尾)** と
> D.1/D.2(SAE同定と懐疑)/D.4(steering)/D.5(judge)。
> **退役(論文に使わない)**: D.0の2軸分類表の「本研究」行(離散編集の
> セル)、D.0bの「本研究」列(事例delta仕様は使うが編集器の話は除く)、
> **D.3全体**(離散編集・Edit Flows・制約付きLevT — EF除外により関連
> しなくなった)、**D.6全体**(ルーティング — routedの除外により消滅)、
> 新規性(a)(b)(d)(条件付け信号 — 使わない。(c)judgeの限界コストと
> R5の(e)(f)が新規性の置き場所)。
> 貢献文: 「**CausalGymの言語学的因果評価をテキスト編集の実行まで拡張し、
