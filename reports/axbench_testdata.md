...
1. **トークン出力EF系の論文からの除外**(出力インターフェース基準):
   ...
   (現提案の学習editorはΔh出力なのでこの除外に該当しないが、下の
   理由2により彼らのベンチには置けない点は同じ。)
2. **除外と独立の原理的問題(2026-07-23 feature-spec移行後の理由に更新)**:
   我々のspecは**minimal pairプールの符号付き平均delta**から構築する。
   AxBenchのベンチ(Concept500系)には最小対が存在せず(概念ラベル+
   例文36と自由生成タスクのみ)、delta specが構築できない。概念ラベル
   だけからspecを作る=概念→latent引き当ては、まさに彼らの検出0.695の
   世界であり、「同定済みプールが与えられたときの因果検証」という我々の
   主張の射程外。(旧理由「事例レベル仕様にtargetが要る」はoracle-spec
   前提 — 現行はtarget不要だがペア構造は必要、と理由が変わった。)
