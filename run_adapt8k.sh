...
# p100 extension to s8000 (user 2026-07-25). Same rule: dev-score
# s6500..8000; if best <= s6000 incumbent (dev 0.4175) -> stop at s6000;
# else eval500 the best new step.
# Run inside interact-g: bash run_adapt8k.sh
...
if [ ! -f "$OUT/eflm-step8000.pt" ]; then
...
for ST in 6500 7000 7500 8000; do
    for DIRX in "" "_amp"; do
...
inc, _ = cell("mixftdev_p100_s6000")
best = None
for st in (6500, 7000, 7500, 8000):
    try:
    ...
    print(f"#  p100 s{st}: net={net:.4f} rmax={rmax:.4f} feas={feas} "
          f"(s6000 inc {inc:.4f})", file=sys.stderr)
    if feas and net > inc and (best is None or net > best[1]):
        best = (st, net)
print(best[0] if best else "STOP6000")
PY
)
echo "[adapt8k] verdict: $SEL"
if [ "$SEL" != "STOP6000" ]; then
    for DIRX in "" "_amp"; do
        if [ -n "$DIRX" ]; then X=--reverse-pairs; else X=""; fi
        O=$P/fs_p100s8kbest_l12$DIRX
        [ -f $O/report.md ] || python scripts/eval_ef_bare.py \
            "${EVC[@]}" --ef-ckpt "$OUT/eflm-step$SEL.pt" \
            --sample-size 500 --output-dir "$O" $X
    done
    for d in $P/fs_p100s8kbest_l12 $P/fs_p100s8kbest_l12_amp; do
