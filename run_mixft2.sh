...
# mixft p50 extension (user-approved): dev net was still rising at the
# s2000 grid edge — continue to 4k steps, score s2500..4000 with the same
# E' relative rule, and eval500 only if a new step beats s2000's dev net.
# Run inside interact-g: bash run_mixft2.sh
...
OUT=$P/eflm_l12_mixft_p50

if [ ! -f "$OUT/eflm-step4000.pt" ]; then
    python train_ef_editor.py \
        --corruption-dir $V4/mixft_p50_l12 \
...
for ST in 2500 3000 3500 4000; do
    for DIRX in "" "_amp"; do
        if [ -n "$DIRX" ]; then X=--reverse-pairs; else X=""; fi
        O=$P/mixftdev_p50_s$ST$DIRX
...
base_net, _ = cell("mixftdev_p50_s2000")     # incumbent (already eval500'd)
best = None
for st in (2500, 3000, 3500, 4000):
    try:
        net, rmax = cell(f"mixftdev_p50_s{st}")
    except Exception:
        continue
    feas = rmax <= refr + 0.015 + 1e-9
    print(f"#  s{st}: net={net:.4f} rmax={rmax:.4f} feas={feas} "
          f"(incumbent s2000 {base_net:.4f})", file=sys.stderr)
    if feas and net > base_net and (best is None or net > best[1]):
        best = (st, net)
print(best[0] if best else "KEEP2000")
PY
)
echo "[mixft2] selected: $SEL"
if [ "$SEL" != "KEEP2000" ]; then
    for DIRX in "" "_amp"; do
        if [ -n "$DIRX" ]; then X=--reverse-pairs; else X=""; fi
        O=$P/fs_mixft2_l12$DIRX
        [ -f $O/report.md ] || python scripts/eval_ef_bare.py \
            "${EVC[@]}" --ef-ckpt "$OUT/eflm-step$SEL.pt" \
            --sample-size 500 --output-dir "$O" $X
    done
    for d in $P/fs_mixft2_l12 $P/fs_mixft2_l12_amp; do
