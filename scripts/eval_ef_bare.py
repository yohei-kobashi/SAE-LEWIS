...
def parse_args():
    ...
    p.add_argument("--a3-edit-prompts", default="",
                   help="per-feature EDIT instructions json "
                        "{feat: {abl:..., enh:...}} (e.g. generated by a "
                        "frontier API model) — replaces the fixed "
                        "template of the prompting_edit arm; random = "
                        "another feature's same-direction instruction.")
...
def main():
    ...
    a3edit = None
    if args.a3_edit_prompts:
        a3edit = json.loads(Path(args.a3_edit_prompts).read_text())
        print(f"[efbare] A3' generated edit prompts: {len(a3edit)} "
              f"features")
        ...
        for c in conditions:
            ...
            for arm in arms:
                ...
                if arm == "ef":
                    ...
                    if c == "true":
                        if a3edit:
                            instr = a3edit[ex["feature"]][dkey]
                        else:
                            instr = (f"Rewrite the input sentence to "
                                     f"{verb} any {feat}. Output only "
                                     f"the rewritten sentence.")
                    elif c == "random":
                        vocab = a3edit if a3edit else a3
                        others = [f2 for f2 in sorted(vocab) if
                                  f2.replace('_', ' ') != feat] \
                            if vocab else ["metaphor"]
                        rf2 = others[int(prng.integers(0, len(others)))]
                        if a3edit:
                            instr = a3edit[rf2][dkey]
                        else:
                            instr = (f"Rewrite the input sentence to "
                                     f"{verb} any "
                                     f"{rf2.replace('_', ' ')}. Output "
