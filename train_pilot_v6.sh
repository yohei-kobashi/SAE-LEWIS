...
# ---- Phase 1: C ingest (PAWS + CoEdIT) ------------------------------------- #
# The v4 cache was generated by parallel workers, so unigram.json lives in
# a worker dir, not at the cache top level — resolve it, then pin it at the
# canonical path for future phases.
# (plain loop, not ls|head: unmatched globs make ls exit nonzero, which
#  set -e -o pipefail turns into a silent death inside $(...))
UNIGRAM=""
for cand in "$V4/corruption/unigram.json" \
            "$V4/corruption_v6pilot/unigram.json" \
            "$V4"/corruption_v6pilot/worker-*/unigram.json \
            "$V4"/corruption_v5topup/worker-*/unigram.json \
            "$V4"/corruption/worker-*/unigram.json; do
    if [ -f "$cand" ]; then UNIGRAM=$cand; break; fi
done
if [ -z "$UNIGRAM" ]; then
    echo "[pilot] no unigram.json found under $V4 — cannot run ingest gates" >&2
    exit 1
fi
echo "[pilot] unigram baseline: $UNIGRAM"
cp -n "$UNIGRAM" "$V4/corruption/unigram.json" 2>/dev/null || true
